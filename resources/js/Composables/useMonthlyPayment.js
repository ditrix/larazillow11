...
export const useMonthlyPayment = (total, interestRate, duration) => {
  ...
  const monthlyPayment = computed(()=> {
    ...
    // Берется основная сумма кредита (цена объекта недвижимости)
    const principle =  isRef(total) ? total.value : total

    // Рассчитывается месячная процентная ставка.
    // interestRate.value — это годовая процентная ставка.
    const monthlyInterest = (isRef(interestRate) ? interestRate.value: interestRate) / 100 / 12

    // Рассчитывается общее количество месяцев, на которые выдается кредит.
